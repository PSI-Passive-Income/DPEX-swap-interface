...
import { BASE_FACTORY_ADDRESS, BASE_INIT_CODE_HASH } from '../constants'
import { computeTradePriceBreakdown } from './prices'

describe('prices', () => {
  const factory: string = BASE_FACTORY_ADDRESS[ChainId.BSCTESTNET]
  ...
  const pair12 = new Pair(new TokenAmount(token1, JSBI.BigInt(10000)), new TokenAmount(token2, JSBI.BigInt(20000)), factory, BASE_INIT_CODE_HASH)
  const pair23 = new Pair(new TokenAmount(token2, JSBI.BigInt(20000)), new TokenAmount(token3, JSBI.BigInt(30000)), factory, BASE_INIT_CODE_HASH)
